public class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
	...
	public int getAux(Position<Entry<K, V>> p) {
		...
		return ((BSTNode<Entry<K, V>>) node).getAux();
	}
	...
	public void rotate(Position<Entry<K, V>> p) {
		...
		if (node == parent.getLeft()) {
			// p is a
			Node<Entry<K, V>> t1 = node.getRight();
			relink(parent, t1, true);
			relink(node, parent, false);
		} else {
			// p is b
			Node<Entry<K, V>> t1 = node.getLeft();
			relink(parent, t1, false);
	...
	public Position<Entry<K, V>> restructure(Position<Entry<K, V>> node) {
		Position<Entry<K, V>> a = parent(node);
		Position<Entry<K, V>> b = parent(a);
		if ((node == right(a)) == (a == right(b))) {
			rotate(a);
			return a;
		} else {
			rotate(node);
